  ...
  useEffect(() => {
    const handleOutsideClick = (e: MouseEvent) => {
      ...
      if (
        ...
        !(e.target as HTMLElement).closest('#btn-nav-mobile-toggle')
      ) {
        setMobileMenuOpen(false);
      }
    };
  ...
  return (
    ...
    >
      ...
      <div 
        ...
        className="w-full bg-[#020b16]/85 backdrop-blur-xl border border-sky-500/25 rounded-full px-2.5 sm:px-4 md:px-5 py-1.5 sm:py-2 flex items-center justify-between gap-1.5 sm:gap-2.5 lg:gap-3 shadow-2xl shadow-black/60 relative min-h-[50px] sm:min-h-[56px]"
      >
            ...
            <div className="flex flex-col justify-center min-w-0">
              <div className="flex items-center gap-1.5 sm:gap-2">
                <span className="font-display font-black text-xs sm:text-sm md:text-base tracking-wider text-white uppercase whitespace-nowrap drop-shadow-sm">
                  <span>SAGAR-SAFE</span>
                  <span className="hidden min-[380px]:inline"> AI</span>
                </span>
                
                {/* Glowing Micro-Badge: COMMAND DECK - VISIBLE ON XL */}
                <span className="hidden xl:inline-block bg-emerald-500/15 text-emerald-400 text-[9px] font-mono font-bold px-1.5 py-0.5 rounded-full border border-emerald-500/30 shadow-[0_0_10px_rgba(16,185,129,0.3)] tracking-wider uppercase shrink-0">
                  DECK
                </span>
              </div>
        ...
        {/* CENTER SECTION: Primary nav — visible on desktop/laptops (lg:flex), horizontal scroll if needed */}
        <nav 
          id="nav-center-capsule"
          aria-label="Command Bridge Primary Navigation"
          className="hidden lg:flex flex-1 min-w-0 items-center justify-center px-1"
        >
          <div
            ref={centerNavScrollRef}
            className="no-scrollbar w-full max-w-2xl min-w-0 overflow-x-auto overscroll-x-contain scroll-smooth touch-pan-x [mask-image:linear-gradient(to_right,transparent,black_8px,black_calc(100%-8px),transparent)] flex justify-center"
          >
            <div className="inline-flex w-max shrink-0 items-center gap-0.5 xl:gap-1 rounded-full border border-slate-800/90 bg-slate-900/80 p-1 shadow-inner backdrop-blur-xl">
            {navLinks.map((link) => {
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `${NAV_PILL} px-2 xl:px-2.5 2xl:px-3 text-[11px] xl:text-xs duration-200 relative z-10 ${
                      isActive
        ...
        <div className="flex shrink-0 items-center justify-end gap-1 sm:gap-1.5">
          ...
          <button
            ...
            className={`hidden md:flex ${NAV_PILL} bg-slate-900/75 hover:bg-slate-800/90 border border-sky-500/30 hover:border-sky-400/60 text-cyan-200 px-2.5 sm:px-3 shadow-sm group max-w-[135px] lg:max-w-[160px] xl:max-w-[180px] 2xl:max-w-[210px] cursor-pointer`}
          ...
          {/* 2. News Dispatch Link — visible on tablet (sm to lg), in center nav on lg+ */}
          <NavLink
            id="btn-nav-news"
            to="/news"
            className={({ isActive }) =>
              `hidden sm:flex lg:hidden ${NAV_PILL} px-2 sm:px-2.5 font-mono font-bold shadow-sm ${
                isActive
          ...
          {onTriggerSos && (
            <button
              ...
              className={`hidden sm:flex ${NAV_PILL} px-2 sm:px-2.5 font-mono font-bold shadow-sm bg-rose-500/20 hover:bg-rose-500/30 border border-rose-500/40 text-rose-300 shadow-[0_0_12px_rgba(244,63,94,0.25)] cursor-pointer`}
            ...
            >
              ...
              <span className="hidden xl:inline leading-none">SOS</span>
            </button>
          )}
          ...
          <button
            ...
            className={`hidden sm:flex ${NAV_PILL} px-2 sm:px-2.5 font-mono font-bold shadow-sm cursor-pointer ${
              isSpeaking
          ...
          <div className="relative shrink-0 hidden sm:block" ref={langMenuRef}>
            <button
              ...
              className={`${NAV_PILL} bg-slate-900/60 hover:bg-slate-800/80 border border-white/15 text-slate-300 hover:text-white px-2 sm:px-2.5 font-mono font-bold cursor-pointer`}
            ...
            {user ? (
              <div>
                ...
                >
                  ...
                  <ChevronDown className={`hidden sm:block w-3 h-3 text-cyan-300 transition-transform ${userMenuOpen ? 'rotate-180' : ''}`} />
                </button>
                  ...
                  >
                    ...
                    <div className="py-1">
                      <button
                        ...
                        className="w-full text-left px-3 py-1.5 text-xs text-slate-300 hover:text-white hover:bg-white/5 rounded-xl flex items-center gap-2 cursor-pointer"
                      >
                      ...
                      <button
                        ...
                        className="w-full text-left px-3 py-1.5 text-xs text-rose-300 hover:text-rose-200 hover:bg-rose-500/15 rounded-xl flex items-center gap-2 cursor-pointer"
                      >
              ...
              <button
                ...
                className={`${NAV_PILL} p-1.5 sm:px-2.5 sm:py-1 bg-white hover:bg-slate-100 text-slate-900 border border-white/20 font-bold shadow-sm hover:scale-[1.02] disabled:opacity-60 cursor-pointer`}
              ...
              >
                ...
                <span className="hidden sm:inline whitespace-nowrap text-xs">Sign In</span>
              </button>
            )}
          </div>
          ...
          {criticalAlertsCount > 0 && (
            <button
              ...
              className="relative h-8 w-8 inline-flex items-center justify-center rounded-full bg-rose-500/20 border border-rose-500/40 text-rose-300 hover:bg-rose-500/30 transition-all focus:outline-none shrink-0 cursor-pointer"
          ...
          {onOpenSettings && (
            <button
              ...
              className="hidden sm:inline-flex h-8 w-8 items-center justify-center rounded-full bg-white/5 hover:bg-white/10 border border-white/10 text-slate-400 hover:text-cyan-200 transition-all focus:outline-none shrink-0 cursor-pointer"
          ...
          {/* 9. RESPONSIVE HAMBURGER MENU TOGGLE (Visible on lg and below) */}
          <button
            id="btn-nav-mobile-toggle"
            type="button"
            onClick={() => setMobileMenuOpen(!mobileMenuOpen)}
            className="lg:hidden h-8 w-8 inline-flex items-center justify-center rounded-full bg-cyan-500/15 hover:bg-cyan-500/25 border border-cyan-400/30 text-cyan-300 transition-all focus:outline-none shrink-0 cursor-pointer"
      ...
      {/* 2. SUB-TELEMETRY STATUS BAR (Single clean balanced row across screen sizes) */}
      <div 
        id="sub-telemetry-bar"
        className="mt-1.5 sm:mt-2 w-full bg-[#020b16]/80 backdrop-blur-md border border-sky-500/20 rounded-full px-2.5 sm:px-4 md:px-5 py-1 sm:py-1.5 flex items-center justify-between text-[10px] sm:text-[11px] font-mono text-slate-300 shadow-xl shadow-black/40 relative overflow-hidden"
      >
        {/* Specular Highlight */}
        ...
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 pr-2">
          ...
          <span className="text-cyan-300 font-bold tracking-wider uppercase truncate text-[10px] sm:text-[11px]">
            <span className="hidden md:inline">MARITIME COMMAND CENTER • </span>FLEET TELEMETRY
          </span>
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-3 shrink-0 text-[10px] sm:text-[11px]">
          ...
          <div className="hidden sm:flex items-center gap-1 sm:gap-1.5 text-slate-200">
            <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-400 animate-pulse ring-2 ring-emerald-500/25 shrink-0" />
            <span className="font-semibold text-slate-200 truncate">Oceansat-3</span>
          </div>

          {/* INCOIS Wave Buoys Beacon */}
          <div className="hidden lg:flex items-center gap-1 sm:gap-1.5 text-slate-200">
            <span className="w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full bg-emerald-400 animate-pulse ring-2 ring-emerald-500/25 shrink-0" />
            <span className="font-semibold text-slate-200 truncate">INCOIS Buoys</span>
          </div>

          {/* Interactive Port Switcher Pill */}
          <button
            type="button"
            onClick={onOpenLocationPicker}
            className="px-2 sm:px-2.5 py-0.5 rounded-full bg-cyan-950/70 hover:bg-cyan-900/80 border border-cyan-500/40 hover:border-cyan-400 text-cyan-200 font-bold text-[9px] sm:text-[10px] uppercase tracking-wider shadow-inner truncate max-w-[130px] sm:max-w-[170px] transition-all flex items-center gap-1 cursor-pointer active:scale-95 shrink-0"
            title={`Active Location: ${selectedLocation.name}. Tap to change harbour.`}
          >
            <span className="text-cyan-400/80">PORT:</span>
            <span className="truncate text-white">{getLocationShortLabel(selectedLocation.name)}</span>
            <ChevronDown className="w-2.5 h-2.5 text-cyan-400 shrink-0" />
          </button>
        </div>
      </div>

      {/* 3. MOBILE & TABLET BACKDROP & RESPONSIVE NAVIGATION DRAWER */}
      {mobileMenuOpen && (
        <div 
          id="nav-mobile-backdrop"
          className="fixed inset-0 bg-black/65 backdrop-blur-sm z-40 lg:hidden animate-fadeIn"
      ...
      {mobileMenuOpen && (
        <div 
          ...
          className="lg:hidden mt-2 max-h-[calc(100dvh-5.5rem)] overflow-y-auto overscroll-contain rounded-2xl sm:rounded-3xl bg-[#020b16]/98 border border-sky-500/40 shadow-2xl p-3.5 sm:p-4 backdrop-blur-2xl animate-fadeIn text-slate-100 z-50 space-y-3 relative"
        >
          {/* Header with status and quick close */}
          <div className="text-[10px] font-mono text-cyan-300/80 uppercase tracking-wider px-1 flex items-center justify-between border-b border-white/10 pb-2.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            ...
            <button
              ...
              type="button"
              onClick={handleToggleGlobalAudio}
              className={`p-2.5 rounded-xl text-xs font-mono font-bold flex items-center justify-center gap-2 border transition-all cursor-pointer ${
                isSpeaking
            ...
            <button
              ...
              className="p-2.5 rounded-xl text-xs font-mono font-bold flex items-center justify-center gap-1.5 bg-slate-900/80 hover:bg-slate-800 text-cyan-200 border border-sky-500/30 hover:border-sky-400 min-w-0 cursor-pointer transition-all"
              title={`Switch from ${selectedLocation.name}`}
            >
              <MapPin className="w-3.5 h-3.5 text-cyan-400 shrink-0" />
              <span className="truncate font-mono">📍 {getLocationShortLabel(selectedLocation.name)}</span>
            </button>
          </div>

          {/* Navigation Items Grid - Responsive Clean 2-column Layout */}
            ...
            {user ? (
              <div className="flex items-center justify-between p-2 rounded-xl bg-cyan-950/50 border border-cyan-400/30">
                ...
                <button
                  ...
                  className="p-2 rounded-lg bg-rose-500/20 text-rose-300 hover:bg-rose-500/30 text-xs font-medium shrink-0 flex items-center gap-1 cursor-pointer transition-colors"
                  title="Sign Out"
                >
              ...
              <button
                ...
                className="w-full py-2.5 px-3 rounded-xl bg-white text-slate-900 text-xs font-bold flex items-center justify-center gap-2 hover:bg-slate-100 shadow-md cursor-pointer transition-all active:scale-[0.99]"
              >
          ...
          <div className="pt-2 border-t border-white/10">
            <div className="text-[10px] font-mono text-slate-400 mb-1.5 uppercase flex items-center gap-1">
              ...
              <span>Coastal Voice Dialect (10 Indic Dialects)</span>
            </div>
            <div className="grid grid-cols-2 sm:grid-cols-5 gap-1.5">
              {languages.map((lang) => (
                <button
                  ...
                  className={`px-2 py-1.5 rounded-xl text-[11px] font-mono transition-all text-center truncate cursor-pointer ${
          ...
          {onTriggerSos && (
            <button
              ...
              className="w-full py-2.5 px-3 rounded-xl bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-bold flex items-center justify-center gap-2 hover:bg-rose-500/30 shadow-[0_0_16px_rgba(244,63,94,0.25)] cursor-pointer transition-all active:scale-[0.99]"
              title={t.navSos || 'Open Maritime Emergency SOS Distress Console'}
            >
          ...
          <div className="flex items-center gap-2 pt-1 border-t border-white/10">
            {onOpenSettings && (
              <button
                ...
                className="flex-1 py-2 px-3 rounded-xl bg-white/5 border border-white/10 text-slate-300 text-xs font-semibold flex items-center justify-center gap-2 hover:bg-white/10 cursor-pointer transition-colors"
              >
            ...
            {criticalAlertsCount > 0 && (
              <button
                ...
                className="flex-1 py-2 px-3 rounded-xl bg-rose-500/20 border border-rose-500/40 text-rose-300 text-xs font-bold flex items-center justify-center gap-2 hover:bg-rose-500/30 cursor-pointer transition-colors"
              >
